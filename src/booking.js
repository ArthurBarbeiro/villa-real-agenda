...
function diasDisponiveis(servicoId, agora = new Date()) {
  ...
  if (!s) return [];
  const agendamentos = db.listarAgendamentos({ status: 'confirmado' });
  return slots.proximosDiasComVaga(s.duracaoMin, agendamentos, config, agora);
}
...
function horariosDisponiveis(servicoId, dataStr, agora = new Date()) {
  ...
  if (!s) return [];
  const agendamentos = db.listarAgendamentos({ status: 'confirmado' });
  return slots.horariosDisponiveis(dataStr, s.duracaoMin, agendamentos, config, agora);
}
