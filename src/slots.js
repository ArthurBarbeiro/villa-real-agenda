...
function candidatosDoDia(dataStr, config) {
  ...
  for (const bloco of blocos) {
    ...
    for (let t = ini; t < fim; t += passo) {
      candidatos.push({ inicioMin: t, blocoFimMin: fim });
    }
  }
  return candidatos;
}
...
function horariosDisponiveis(dataStr, duracaoMin, agendamentos, config, agora = new Date()) {
  ...
  for (const c of candidatos) {
    ...
    // 1) o serviço tem que caber dentro do bloco de funcionamento
    if (fim > c.blocoFimMin) continue;
...
function proximosDiasComVaga(duracaoMin, agendamentos, config, agora = new Date()) {
  const dias = [];
  const hoje = dataLocalISO(agora);
  for (let i = 0; i <= config.regras.diasParaFrente; i++) {
    const dataStr = somarDias(hoje, i);
    const livres = horariosDisponiveis(dataStr, duracaoMin, agendamentos, config, agora);
    if (livres.length > 0) {
