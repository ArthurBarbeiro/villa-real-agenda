...
document.querySelectorAll('.tab').forEach((tab) => {
  tab.addEventListener('click', () => {
    ...
    if (alvo === 'barbeiro') {
      if (ADMIN_PIN) { carregarAgenda(); carregarBloqueios(); }   // já autenticado
...
async function carregarAgenda() {
  ...
  // Estado de bloqueio do dia (folga/imprevisto) — atualiza o botão de bloquear
  let bloqueado = false;
  try { const st = await api(`/api/admin/bloqueios/${iso}`); bloqueado = !!st.bloqueado; } catch (_) {}
  atualizarBotaoBloqueio(iso, bloqueado);

  try {
    const lista = await api(`/api/agendamentos?data=${iso}`);
    box.innerHTML = '';
    if (bloqueado) {
      box.appendChild(el('<div class="card" style="border:1px solid var(--danger);background:rgba(192,57,43,.12);margin:0 0 10px">'
        + '<strong style="color:var(--danger)">🚫 Dia bloqueado</strong><br>'
        + '<small class="muted">Este dia não aceita novos agendamentos. Toque em “Desbloquear este dia” para reabrir.</small></div>'));
    }
    if (!lista.length) {
      box.appendChild(el('<p class="muted center" style="padding:20px 0">Nenhum horário marcado neste dia.</p>'));
      return;
    }
...
// ---------- Bloqueio de dia inteiro (folga / imprevisto) ----------
function atualizarBotaoBloqueio(iso, bloqueado) {
  const btn = document.getElementById('btn-bloquear-dia');
  if (!btn) return;
  if (bloqueado) {
    btn.textContent = '✅ Desbloquear este dia';
    btn.classList.add('secondary');
    btn.onclick = () => desbloquearDia(iso);
  } else {
    btn.textContent = '🚫 Bloquear este dia inteiro';
    btn.classList.remove('secondary');
    btn.onclick = () => bloquearDiaAtual(iso);
  }
}

async function bloquearDiaAtual(iso) {
  // Avisa se já houver agendamentos nesse dia
  let qtd = 0;
  try { const lista = await api(`/api/agendamentos?data=${iso}`); qtd = lista.length; } catch (_) {}
  let msg = `Bloquear o dia ${formatarDataBR(iso)} inteiro?\n\nOs clientes não vão conseguir marcar horários nesse dia.`;
  if (qtd > 0) {
    msg += `\n\n⚠️ Já existem ${qtd} agendamento(s) nesse dia. Eles NÃO serão cancelados automaticamente — avise os clientes se precisar.`;
  }
  msg += `\n\n(Opcional) Escreva o motivo da folga, ou deixe em branco:`;
  const motivo = prompt(msg, '');
  if (motivo === null) return; // cancelou
  try {
    await api('/api/admin/bloqueios', { method: 'POST', body: JSON.stringify({ data: iso, motivo: motivo || '' }) });
    toast('Dia bloqueado.');
    carregarAgenda();
    carregarBloqueios();
  } catch (e) { toast(e.message); }
...
async function desbloquearDia(iso) {
  try {
    await api(`/api/admin/bloqueios/${iso}`, { method: 'DELETE' });
    toast('Dia reaberto.');
    carregarAgenda();
    carregarBloqueios();
  } catch (e) { toast(e.message); }
...
async function carregarBloqueios() {
  const box = document.getElementById('lista-bloqueios');
  if (!box) return;
  try {
    const lista = await api('/api/admin/bloqueios');
    if (!lista.length) { box.innerHTML = '<p class="muted center" style="padding:8px 0">Nenhum dia bloqueado.</p>'; return; }
    setKids(box,);
    lista.forEach((b) => {
      const dow = NOMES_DIAS[diaSemanaISO(b.data)];
      const motivo = b.motivo ? ' · ' + b.motivo : '';
      const aviso = b.agendamentos > 0
        ? `<br><small style="color:var(--danger)">⚠️ ${b.agendamentos} agendamento(s) já marcados nesse dia</small>` : '';
      const item = el(`
        <div class="ag-item">
          <div class="ag-info">
            <strong>${dow}, ${formatarDataBR(b.data)}</strong>
            <small>Bloqueado${motivo}</small>${aviso}
          </div>
          <button class="btn secondary">Reabrir</button>
        </div>`);
      item.querySelector('button').addEventListener('click', () => desbloquearDia(b.data));
      box.appendChild(item);
    });
  } catch (e) { box.innerHTML = `<p class="muted">${e.message}</p>`; }
}
